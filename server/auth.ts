...
declare global {
  namespace Express {
    // Define the user interface to match our User type
    interface User {
      id: number;
      username: string;
      password: string;
...
export function setupAuth(app: Express) {
  ...
  const isAdmin = (req: Request, res: Response, next: NextFunction) => {
    if (!req.isAuthenticated() || req.user.role !== 'admin') {
      return res.status(403).json({ message: "Accesso vietato" });
    }
    next();
  };
  ...
  const isDoctor = (req: Request, res: Response, next: NextFunction) => {
    if (!req.isAuthenticated() || (req.user.role !== 'doctor' && req.user.role !== 'admin')) {
      return res.status(403).json({ message: "Accesso vietato" });
    }
    next();
  };
  ...
  return { isAdmin, isDoctor, isAuthenticated };
}
